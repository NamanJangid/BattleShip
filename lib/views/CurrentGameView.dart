...
class CurrentGameView extends StatefulWidget {
  ...
  CurrentGameView({super.key, required this.gameId, this.isCompleted = false});
...
class _CurrentGameViewState extends State<CurrentGameView> {
  ...
  bool myTurn = true; // Assume it's the user's turn initially
  ...
  Future<void> _showMyDialog(bool didIWin) async {
    return showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text("${didIWin ? 'You' : 'Opponent'} Won!!!"),
            actions: [
              TextButton(
                  onPressed: () {
  ...
  Future<void> _getResponse() async {
    ...
    if (answer['ships'] != null) {
      ...
      if (widget.isCompleted) {
        _showMyDialog(answer['status'] == answer['position']);
      }
    }
    if (answer['position'] == answer['turn']) {
      shouldBeGiven = true;
      myTurn = true; // Update myTurn based on the turn position
    } else {
      shouldBeGiven = false;
      myTurn = false; // Update myTurn based on the turn position
    }
  }
  ...
  void _submitButton() async {
    ...
    if (newResponse.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Retry again after some time!!!")));
    }
  }
